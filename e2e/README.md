...
Screenshots (when tests run) are written under `e2e/test-results/screenshots/` (smoke flow) and `e2e/test-results/ci-visual/` ([`tests/ci-regression.spec.ts`](tests/ci-regression.spec.ts): admin panel, font size, display filters, empty-search field sanity, syslog grid & cell menu, `severity:warning` filter submit, plaintext export download, statistics/reports/advanced search pages, optional predefined-search menu). Both trees are uploaded with the **`playwright-e2e`** GitHub Actions artifact.
